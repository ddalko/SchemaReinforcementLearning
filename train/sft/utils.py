...
def preprocess_dataset(data, tokenizer, max_length, test_split=0.9):
    preprocessed_data = []
    for i in tqdm(range(len(data)), desc="Preprocessing dataset"):
        prompt = [data[i]["messages"][0]]
        response = [data[i]["messages"][1]]
    ...
    eval_data = preprocessed_data[int(len(preprocessed_data)*test_split):]
